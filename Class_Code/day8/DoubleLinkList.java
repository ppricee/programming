 ...
 */
public class DoubleLinkList<K extends Comparable<K>, V>{
    int listsize;
    int current;
    DCell<K,V> head;
    DCell<K,V> tail;
    ...
    public void insert(int idx, K k, V v){
        DCell<K,V> Dcell = new DCell();
        DCell<K,V> after = null;
        Dcell.setValue(v);
        Dcell.setKey(k);
        if (listsize == idx){
            append(k, v);
            ...
            DCell<K, V> before = after.getPrev();
    ...
    public void remove(int idx){
        DCell<K, V> after = null;
        DCell<K, V> delete = null;
        DCell<K, V> before = null;
        for (int i = 0; i <= idx; i++){
            if(i == 0){
                delete = head;
            }
...
//        insert(didx, cell.getValue());
     ...
     */
    public V fetch(K k){
        DCell<K, V> fetch = null;
        for(int i = 0; i < listsize; i++){
            if (i == 0){
                fetch = head;
            }
            else if(k == fetch.getKey()){
                break;
            }
            else{
                fetch = fetch.getNext();
    ...
    /**
     * Fetches the key at the specific index
     * @param idx
     * @return a key
     */
